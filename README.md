# Raincord 100k Gallery Region
A gallery region celebrating 100k members in the [Rain World Discord Server](https://discord.gg/rainworld)! Each room is made by a member of the community and themed after an in-game region.

### Featuring
* 920 screens across 323 rooms by 126 different people
* 75 custom pearls written by 53 different people
* 5 custom tracks by 4 different people
* Custom threat theme mash up of various other threat themes
* 47 shelters
* Custom object: the peanut! You'll find it occasionally throughout the regions.
* Custom slugcat to explore with. It comes with a starting region picker and can read different readings of pearls without having to worry about iterators!

### Requirements
* Downpour DLC
* Watcher DLC
* [Custom Regions Support](https://github.com/Bro748/Custom-Regions)
* [POM](https://github.com/Rain-World-Modding/POM)
* [RegionKit](https://github.com/Rain-World-Modding/RegionKit)
* Floodwaters Utilities
* Chroma's Tools and Stuff
* [Slugbase](https://github.com/SlimeCubed/SlugBaseRemix)
* [Fisobs](https://github.com/Dual-Iron/fisobs)
* [The M4rblelous Entity Pack](https://github.com/M4rbleL1ne/The-M4rblelous-Entity-Pack)
* Alduris's Entity Pack
* [FUtility](https://github.com/Rain-World-Modding/FUtility)

### Notes
* Since this is a gallery region made by people of all sorts of experience levels, do not expect gameplay to be polished or looks to be coherent in any way. You have been warned. Please do not harass anyone involved in this region.
* This region contains region spoilers for all campaigns in vanilla and all DLC (obviously).
* The region has spawns that include some custom creatures and have lineages, but they have not been playtested much. Let me know if it is too much and I might tweak them.
* I probably won't be fixing most issues. I can change code things, placed objects, and spawns, but I cannot do anything about stuff like bad camera placements in rooms (of which there are several).
* Yes it is missing select screen art. I have no plans to add it for now.

Have fun!! :monksilly:

# Credits
## Room submissions
* 0rchid
* 3Circles
* 500mils
* Addy
* Alduris
* Alice Pigeon
* Ant
* Anubisrah
* AprilWORSTmonth
* Aries_Client
* AV3RY
* Bat
* Bismuth
* Blobtic
* BLU
* BuzzyBro
* Chloe
* ChromaticalBlight
* Clapom
* CocoaRaptor
* crafterz_
* CreativeGamer03
* Crimsont04
* Dakras
* Damoonlord
* Dedniko
* Detrax
* DietPebsi
* Dododecalogist
* Dragonly44
* DragonPropaganda
* EmptyRaider101
* EthanWantsFood
* Fanc39cat
* FatalError
* Five Frozen Lakes
* FlanAce
* Flarf
* FluffSys
* Forthfora
* Fowl
* Frogturtle56
* FunkyTrashCan
* gabkai11
* gal3o
* GaleTheCat
* gamerguy35
* GonterHaoz
* Haizlbliek
* hallowfish
* Haysen
* hennyburgr
* Hobart
* Inspectnerd
* IOI
* Irri
* Iterator-iterator
* Izuna
* Jachii
* January
* Javadog
* Kalum
* Kato
* Kem
* Kizorke
* Korii
* Lkelly
* LuckyKitKat
* Lunatav
* Luniular
* lwrcsv
* MagicaJaphet
* Mataneko
* MaxTheLord
* Mk-0
* MrPtata
* Myrmice
* Nimble
* Nocturne
* noelleCK
* Nopuddles
* Nova
* Osziaq
* OverpoweredPizza
* Ovidia
* Pelican
* PrestoPesto
* Puffball
* Pur
* Pykrite
* RandomDirection
* Rbor
* Reincarnid
* Renz
* RolinGP3
* Rookie
* Rose
* Rostik630
* ScoutRiley
* Sekq
* Slithersss
* Sniffledorf
* Solar
* SteellessToaster
* Stero
* Tankium
* Tapok
* TetraNeo
* TheCubifyer
* thenotsogoodgamer
* Thysi
* TotallyDutch
* Trearilt
* Tronsx
* Tryn
* tvfella
* user23848
* Viv
* Walekks
* Wonk
* Wrayk
* Yonko
* Zamtreon
* Zaraza
* Zarquon
* Zenodos

## Music makers
* AV3RY
* EthanWantsFood
* Renz
* Sequoia7

## Decal artists
* AV3RY
* Bat
* CreativeGamer03
* Fanc39cat
* GonterHaoz
* noelleCK
* Nova
* Pines
* Rbor
* SadBear
* TheLexDex
* Walekks
* Wonk
* Zaraza

## Writing submissions
* 3Circles
* Aetherius
* After_T13
* Alduris
* Anton
* AV3RY
* CeleBecca
* Clapom
* Complex_City
* Cy
* Domez the Dragon
* DrFunkk
* Drymm
* dumpstirfire
* dylanplayz2286
* Equino
* FunkyTrashCan
* germtlemaam
* Grey_Skyies
* Halves
* Heldensichel
* imbadatusernamesokay
* JustAGuy2
* Lode2
* lowflyingplains
* Luniular
* M-Cool
* Martes
* michiru6294
* Mokie_Voidcat
* Mr. Karl
* Pickle
* Pinata
* Piotr08
* Plasmoidman
* Popsicle
* Reincarnid
* RNolis
* Rotwall
* Slythy
* Smudge
* Solaris Owl
* Soulwing98
* Soup Moth
* sparkproot
* Spyrozyde
* Tarr
* TheBurgerBox
* timelostprotodrake
* T_The_Artist
* Wonk
* Zedarel
* Zombieboy139
